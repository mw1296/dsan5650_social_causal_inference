...
display(pd.DataFrame(az.summary(race_idata)))
```
@tbl-race-model-params displays information (mu_population, beta, p_unemployment) about the estimated posterior distribution over values of the race model's parameters.

```python
# Chcek train posterior mean vs. mena of survey train data
print(f'posterior_unemployment_mean on train data: {race_idata.posterior.p_unemployment.mean().values:.5f}')

print(f'unemployment_mean on train data: {y_train.mean():.5f}')
```
...
The PGM posterior predictive unemployement mean on train data is very close to the calculated unemployment mean
...
display(pd.DataFrame(az.summary(race_postpred_idata)).head(10))
```
@tbl-race-model-posterior-distribution shows a sample of posterior preditive distribution on train data
...
with unemployment_race_model:
    pm.set_data({"unemployment_obs": y_test,}, coords=test_coords)
with unemployment_race_model:
...
```{python}
# Extract means of sayhire values from the posterior predictive distribution given test data
race_test_meanunemployment = nofeats_test_idata.posterior_predictive['unemployment'].mean(dim=['chain','draw'])
...
```{python}
## 4.Visualize the PGM
display(pm.model_to_graphviz(adaptive_unemployment_industry_model))
```
...
#| tbl-cap: "Summary of Posterior Distributions"
#| tbl-col: true
#| tbl-center: true
