...
### Age_Education model - Survey data of age, education attainment and employment status
Create df_age_education using 2025 year to date survey data

```python
survey_file_names = ['jan_2025.csv', 'feb_2025.csv', 'mar_2025.csv', 'apr_2025.csv', 'may_2025.csv', 'jun_2025.csv', 'jul_2025.csv']
...
# Exclude observations with doctoral degree since 22-25 age is quite unlikely to receive doctoral degree
survey_df_age_education = survey_df_age_education[survey_df_age_education['education_attainment_description']!='doctoral_degree']

survey_df_age_education['education_attainment_group'] = np.where(
    survey_df_age_education['education_attainment_description'] == 'masters_degree',
    'masters',
    np.where(
        survey_df_age_education['education_attainment_description'] == 'bachelors_degree',
        'bachelors',
...
survey_df_age_education['is_bachelors'] = (survey_df_age_education['education_attainment_group']=='bachelors').astype(int)

survey_df_age_education['is_masters'] = (survey_df_age_education['education_attainment_group']=='masters').astype(int)

survey_df_age_education['is_lower_than_bachelors'] = (survey_df_age_education['education_attainment_group']=='lower than bachelors').astype(int)
...
### Age_education model - train-test split
```python
# train-test split
...
### Age_education model - define the multivariate model
...
with pm.Model(coords=coords) as age_education_model:
    ...
    lower_than_bachelors_obs = pm.Data('is_lower_than_bachelors_obs', X_train_ae['is_lower_than_bachelors'], dims='response_id')
    # Prior of population mean log odds
    mu_population_log_odds = pm.Normal('mu_population_log_odds', mu=population_mean_log_odds, sigma=0.5)
    ...
    beta_lower_than_bachelor = pm.Normal("beta_lower_than_bachelor", 0, 1)
    ...
    beta_recent_graduate_lower_than_bachelor = pm.Normal("beta_recent_graduate_lower_than_bachelor", 0, 1)
    # p_unemployment determined by variables
    p_unemployment = pm.Deterministic("p_sayhire", pm.math.invlogit(mu_population_log_odds + beta_bachelor * bachelors_obs + beta_master * masters_obs + beta_lower_than_bachelor * lower_than_bachelors_obs + beta_recent_graduate * recent_graduate_obs + beta_recent_graduate_bachelor * recent_graduate_obs * bachelors_obs + beta_recent_graduate_master * masters_obs * recent_graduate_obs + beta_recent_graduate_lower_than_bachelor * lower_than_bachelors_obs * recent_graduate_obs), dims = 'response_id')
    # unemployment
    unemployment_obs = pm.Data("unemployment_obs", y_train, dims="response_id")
    unemployment = pm.Bernoulli("unemployment", p = p_unemployment, observed=unemployment_obs, dims="response_id")
```
...
### Age_education model - Estimate model parameters
