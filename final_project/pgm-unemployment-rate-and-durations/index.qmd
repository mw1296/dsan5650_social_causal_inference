...
```python
# Create scatter plot to visualize pred y vs. true y
...
sns.scatterplot(
    x = race_test_idata.posterior_predictive['unemployment'].mean(dim=['chain','draw']).values, # predictive unemployment rate
    y = race_test_idata.posterior_predictive['unemployment'].mean(dim=['chain','draw']).values,
    hue=race_list, 
    palette="Set1",
    ax=ax
)
