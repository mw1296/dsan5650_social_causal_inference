...
with pm.Model() as unemployment_race_model:
    ...
    # Predict on observed data: use parameters above to predict probabilities of unemployment using June 2025 survey data.
...
with pm.Model(coords=train_coords) as unemployment_race_model:
    ...
    # Assign the correct probability to p_individual based on race
    p_unemployment = pm.math.switch(
        X_train['is_black_african'].values,
        p_black, # if is_black, use black prob
        pm.math.switch(
            X_train['is_asian'].values, 
            p_asian, # if is_asian, use asian prob
            p_white # else, use white prob
        )
    )
    ...
    #     'individual_status_obs',
    #     p=p_individual_for_obs,
    #     observed=X_train['unemployment_status'].values
    # )

    unemployment_obs = pm.Data("employment_status_obs", y_train, dims="response_id")

    unemployment = pm.Bernoulli("employment_status", p = p_unemployment, observed=unemployment_obs, dims="response_id")
```
